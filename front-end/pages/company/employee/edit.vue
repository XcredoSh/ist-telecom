          ...
          <div class="mt-2">
            <form @submit.prevent="registerEmployee">
              <div class="flex flex-col md:flex-row border-b border-gray-200 pb-4 mb-4">
                <div class="w-64 font-bold h-6 mx-2 mt-3 text-gray-800">Xodim {{employee_id}}</div>
              ...
              <div class="flex flex-col md:flex-row">
                ...
                <div class="flex-1 flex flex-col md:flex-row">
                  <button type="submit" class="text-sm  mx-2 w-32  focus:outline-none flex justify-center px-4 py-2 rounded font-bold cursor-pointer
                    ...
                    border-teal-600 transition">Yangilash
                  </button>
                </div>
              </div>
            </form>
...
export default {
  ...
  methods: {
    ...
    async registerEmployee() {
      try {
        await this.$axios.post('/employees', this.employeeData).then((res) => {
          console.log(res);
          this.$toast.show({
            type: 'success',
            title: 'Success',
